...
while t <= n:
    ...
    """
    reward[t], attackornot, hj, ij = att.judge(maintree[nodeiter].represent, t, K)
    if hj != ht or ij != it:
        print("judge error!\n")
        exit(0)
    ...
    if K >= comtree[comnodeiter].start and K < comtree[comnodeiter].end:
        ...
        comreward[t] = random.gauss(minv, 0.5)
        comattsum[t] = comattsum[t-1] + 1
...
fpnodenum = open("./"+sys.argv[1]+"/nodenum","w")
fpnodenum.write(str(len(maintree)))
fpnodenum.write("\n")
fpnodenum.write(str(len(comtree)))
fpnodenum.write("\n")
fpnodenum.write(str(len(rantree)))
fpnodenum.write("\n")
fpnodenum.write(str(len(hcttree)))
fpnodenum.write("\n")
fpnodenum.close()

fpregret = open("./"+sys.argv[1]+"/regret","w")
...
#print(hmax)
ax.plot(timelist, regret,linestyle='--',linewidth=2)
...
axins.plot(timelist, attsum, linestyle='--',linewidth=2)
axins.plot(timelist, comattsum, color='red', linestyle='-.',linewidth=2)
axins.set_xlim(int(0.8e7), n)
axins.set_ylim(min(attsum[int(0.8e7)],comattsum[int(0.8e7)])-5000,max(attsum[n],comattsum[n])+5000)
